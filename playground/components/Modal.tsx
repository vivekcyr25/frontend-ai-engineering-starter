...
export function Modal({
  ...
  // Ensure portal target is available on client
  useEffect(() => {
    setMounted(true);
  }, []);

  // Capture active element before modal opens & manage focus entry and restoration
  useEffect(() => {
    if (!isOpen) return;

    // Store element that had focus when dialog opened
    ...
    return () => {
      ...
      // Focus Restoration: return focus to trigger or finalFocusRef
  ...
  // Keyboard trap (Tab / Shift+Tab) & Escape key dismissal
  useEffect(() => {
    if (!isOpen) return;
